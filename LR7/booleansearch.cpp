...
bool is_term_like(const string& s) {
    return !(s == "(" || s == ")" || s == "AND" || s == "OR" || s == "NOT");
}

vector<string> split_query_lexemes(const string& query) {
    vector<string> out;
    string cur;
...
vector<string> normalize_query_terms(const vector<string>& raw_tokens) {
    vector<string> out;

    for (const string& tok : raw_tokens) {
        if (tok == "(" || tok == ")" || tok == "AND" || tok == "OR" || tok == "NOT") {
            out.push_back(tok);
            continue;
        }

        vector<string> parts = tokenize_text(tok);
...
vector<string> insert_implicit_and(const vector<string>& tokens) {
    vector<string> out;
    ...
    for (size_t i = 1; i < tokens.size(); ++i) {
        const string& prev = tokens[i - 1];
        const string& cur = tokens[i];

        bool prev_can_end_expr = is_term_like(prev) || prev == ")";
        bool cur_can_start_expr = is_term_like(cur) || cur == "(" || cur == "NOT";

        if (prev_can_end_expr && cur_can_start_expr) {
            out.push_back("AND");
        }
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        vector<string> raw = split_query_lexemes(query);
        vector<string> norm = normalize_query_terms(raw);
        vector<string> expanded = insert_implicit_and(norm);
        vector<string> rpn = to_rpn(expanded);
